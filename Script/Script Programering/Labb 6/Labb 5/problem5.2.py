...
def simple_calculater ():
    ...
    while operation != "quit": #keep looping as long as operation is not equal to "quit"
        ...
        if operation == "add":
            ...
            with open (r"c:\script\calculator.txt", "w") as math_xml:
                store_value = value.copy()
                math_xml.write(str(store_value))

        elif operation == "load":
            with open (r"c:\script\calculator.txt", "r") as load_xml:
                file_content = load_xml.read()
                value = eval(file_content)
